...
export class ImageCropperComponent implements OnChanges, OnInit {
    ...
    private transformedImage: HTMLImageElement | null = null;
    private originalBase64: any;
    private transformedBase64: string | undefined;
    private moveStart: MoveStart | undefined;
    private originalSize: Dimensions | undefined;
    private transformedSize: Dimensions | undefined;
    ...
    maxSize: Dimensions | undefined;
    imageVisible = false;
    moveTypes = MoveTypes;

    @ViewChild('wrapper', { static: true }) wrapper: ElementRef | undefined;
    @ViewChild('sourceImage', { static: false }) sourceImage:
        | ElementRef
        | undefined;
    ...
    @Input() format: 'png' | 'jpeg' | 'bmp' | 'webp' | 'gif' | 'ico' = 'png';
    ...
    private loadImageFile(file: File): void {
        ...
        fileReader.onload = (event: any) =>
            this.loadImage(event.target.result, file.type);
    ...
    private loadBase64Image(imageBase64: string): void {
        this.autoRotateSupported
            .then((supported: boolean) =>
                this.checkExifAndLoadBase64Image(imageBase64, supported)
            )
    ...
    private checkExifAndLoadBase64Image(
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            this.originalImage.onload = () => {
                ...
                this.exifTransform = getTransformationsFromExifData(
                    autoRotateSupported ? -1 : imageBase64
                );
                this.originalImage!.style.maxHeight = '100px';
    ...
    private setTransformedImage(transformedBase64: any): Promise<void> {
        return new Promise<void>((resolve) => {
            ...
            this.transformedImage.onload = () => {
                this.transformedSize!.width =
                    this.transformedImage!.naturalWidth;
                this.transformedSize!.height =
                    this.transformedImage!.naturalHeight;
                this.cd.markForCheck();
                resolve();
            };
            (this.transformedImage as any).src = this.transformedBase64;
        });
    }
    ...
    private checkImageMaxSizeRecursively(): void {
        if (this.setImageMaxSizeRetries > 40) {
            ...
            this.setCropperScaledMinSize();
            this.resetCropperPosition();
            this.cropperReady.emit({ ...(this.maxSize as any) });
            this.cd.markForCheck();
        } else {
    ...
    resetCropperPosition(): void {
        ...
        if (this.cropperStaticHeight && this.cropperStaticWidth) {
            this.cropper.x1 = 0;
            this.cropper.x2 =
                sourceImageElement.offsetWidth > this.cropperStaticWidth
                    ? this.cropperStaticWidth
                    : sourceImageElement.offsetWidth;
            this.cropper.y1 = 0;
            this.cropper.y2 =
                sourceImageElement.offsetHeight > this.cropperStaticHeight
            ...
            if (!this.maintainAspectRatio) {
                ...
                const cropperHeight =
                    sourceImageElement.offsetWidth / this.aspectRatio;
                this.cropper.y1 =
                    (sourceImageElement.offsetHeight - cropperHeight) / 2;
                this.cropper.y2 = this.cropper.y1 + cropperHeight;
            } else {
                this.cropper.y1 = 0;
                this.cropper.y2 = sourceImageElement.offsetHeight;
                const cropperWidth =
                    sourceImageElement.offsetHeight * this.aspectRatio;
                this.cropper.x1 =
                    (sourceImageElement.offsetWidth - cropperWidth) / 2;
    ...
    private setMaxSize(): void {
        if (this.sourceImage) {
            ...
            this.marginLeft = this.sanitizer.bypassSecurityTrustStyle(
                'calc(50% - ' + this.maxSize!.width / 4 + 'px)'
            );
        }
    }
    ...
    private setCropperScaledMinWidth(): void {
        this.cropperScaledMinWidth =
            this.cropperMinWidth > 0
                ? Math.max(
                      20,
                      (this.cropperMinWidth / this.transformedImage!.width) *
                          this.maxSize!.width
                  )
                : 20;
    }
    ...
    private setCropperScaledMinHeight(): void {
        if (this.maintainAspectRatio) {
            ...
            this.cropperScaledMinHeight = Math.max(
                20,
                (this.cropperMinHeight / this.transformedImage!.height) *
                    this.maxSize!.height
            );
        } else {
            this.cropperScaledMinHeight = 20;
    ...
    private resize(event: any): void {
        ...
        switch (this.moveStart!.position) {
            ...
            case 'center':
                ...
                const newWidth =
                    Math.abs(this.moveStart!.x2 - this.moveStart!.x1) * scale;
                const newHeight =
                    Math.abs(this.moveStart!.y2 - this.moveStart!.y1) * scale;
                const x1 = this.cropper.x1;
                const y1 = this.cropper.y1;
                this.cropper.x1 = Math.min(
                    this.moveStart!.clientX - newWidth / 2,
                    this.cropper.x2 - this.cropperScaledMinWidth
                );
                this.cropper.y1 = Math.min(
                    this.moveStart!.clientY - newHeight / 2,
                    this.cropper.y2 - this.cropperScaledMinHeight
                );
                this.cropper.x2 = Math.max(
                    this.moveStart!.clientX + newWidth / 2,
    ...
    private checkAspectRatio(): void {
        ...
        switch (this.moveStart!.position) {
            ...
            case 'center':
                ...
                const overflowX2 = Math.max(
                    this.cropper.x2 - this.maxSize!.width,
                    0
                );
                const overflowY1 = Math.max(
                    this.cropper.y2 - this.maxSize!.height,
                    0
                );
                ...
                if (
                    ...
                    this.cropper.x1 +=
                        overflowY1 * this.aspectRatio > overflowX1
                            ? overflowY1 * this.aspectRatio
                            : overflowX1;
                    this.cropper.x2 -=
                        overflowY2 * this.aspectRatio > overflowX2
                            ? overflowY2 * this.aspectRatio
                            : overflowX2;
                    this.cropper.y1 +=
                        overflowY2 * this.aspectRatio > overflowX2
                            ? overflowY2
                            : overflowX2 / this.aspectRatio;
                    this.cropper.y2 -=
                        overflowY1 * this.aspectRatio > overflowX1
                            ? overflowY1
                            : overflowX1 / this.aspectRatio;
                }
                break;
        }
    }
        ...
        if (
            ...
            if (ctx) {
                ...
                ctx.setTransform(
                    ...
                    0,
                    scaleY,
                    this.transformedSize!.width / 2,
                    this.transformedSize!.height / 2
                );
                ctx.translate(
                    -imagePosition.x1 / scaleX,
                    -imagePosition.y1 / scaleY
                );
    ...
    private getImagePosition(): CropperPosition {
        const sourceImageElement = this.sourceImage!.nativeElement;
        const ratio =
            this.transformedSize!.width / sourceImageElement.offsetWidth;
